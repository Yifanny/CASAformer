...
def weighted_masked_mae_loss(preds, labels, null_val=0.0):
    if np.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = labels != null_val
    ...
    batch_size, seq_len, n_nodes, _ = labels.size()
    label_mean = torch.mean(labels, 1).squeeze(-1)
    for threshold in range(20, 60, 10):
        if threshold == 20:
            congest_idx = torch.argwhere(torch.logical_and(0 < label_mean, label_mean <= threshold))
        else:
            congest_idx = torch.argwhere(torch.logical_and(threshold - 10 < label_mean, label_mean <= threshold))
        if threshold <= 30:
            # weight = 1 / threshold * 400 # for METRLA
            # weight = 1 / threshold * 600 # for PEMSBAY
            weight = 1 / threshold * 60
        else:
            # weight = 1 / threshold * 200 # for METRLA
            # weight = 1 / threshold * 300 # for PEMSBAY
            weight = 1 / threshold * 60
        loss[congest_idx[:, 0], :, congest_idx[:, 1], :] = loss[congest_idx[:, 0], :, congest_idx[:, 1], :] * weight
...
class WeightedMaskedMAELoss:
    def _get_name(self):
        return self.__class__.__name__

    def __call__(self, preds, labels, null_val=0.0):
        return weighted_masked_mae_loss(preds, labels, null_val)
