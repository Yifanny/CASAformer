...
def test_model(model, testset_loader, log=None, phase="all"):
    ...
    gt = []
    pred = []
    for i in range(0, 6850 - 12, 12):
        gt.append(y_true[i, :, 0])
        pred.append(y_pred[i, :, 0])
    print(np.array(gt).shape, np.array(pred).shape)
    gt = np.array(gt).flatten()
    pred = np.array(pred).flatten()
    print(gt.shape, pred.shape)
    for i in range(0, len(gt), 288):
        plt.figure()
        plt.plot(range(len(gt[i: i + 288])), gt[i: i + 288], color="blue")
        plt.plot(range(len(pred[i: i + 288])), pred[i: i + 288], color="red")
        plt.savefig("node1"+str(int(i/288))+".png")
    exit()
    """
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("../saved_models/mySTAEformer-0x-0x1-sparse_att_weighted_loss-METRLA-2024-07-21-21-41-36.ptweighted", map_location=DEVICE))
    model.to(DEVICE)
